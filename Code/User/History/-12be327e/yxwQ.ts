...
export class TemplateService {
  ...
  async getTemplateInformations(
    template: TemplateEntity,
  ): Promise<TemplateInformationsResponse> {
    const queryBuilder = this.dataSource
      .getRepository(TemplateEntity)
      .createQueryBuilder('templates')
  ...
  async getCardsFromTemplate(templateId: number, query: any){

    const queryBuilder = this.dataSource
      .getRepository(CardEntity)
      .createQueryBuilder('cards');

    queryBuilder.andWhere('cards.template = :id', {
      id: `${templateId}`,
    });
    queryBuilder.orderBy('cards.id', 'DESC');
    ...
    return {
      cardCount: cardCount,
      cards: await queryBuilder.getMany(),
    };
  }

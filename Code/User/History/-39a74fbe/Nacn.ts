...
import * as buffers from 'node:buffer';
...
export class CardService {
  ...
  async exportCard(id: number, query: any): Promise<any> {
    ...
    //@ts-ignore
    const frontImageBase64 = templateInformations.front.background;

  
    const jpgImageBytes = this.base64ToArrayBuffer(frontImageBase64);
    const jpgImage = await pdfDoc.embedJpg(jpgImageBytes)
  
    frontPage.setFontColor(rgb(frontColor.r, frontColor.g, frontColor.b));
    frontPage.drawImage(jpgImage);
  ...
  private base64ToArrayBuffer(base64: string): ArrayBuffer {
    const binaryString = buffers.Buffer.from(base64, 'base64').toString('binary');

...
export class CardService {
  ...
  async exportCard(id: number): Promise<any> {
    ...
    await this.writeToPage(pdfDoc, frontPage, templateInformations.front.informations, card.data.front)
    ...
    await this.writeToPage(pdfDoc, backPage, templateInformations.back.informations, card.data.back)
    ...
    for (const key in template) {
      ...
      switch (value.type) {
        ...
        case 'image':
            ...
            const imageBuffer = await pdfDoc.embedPng(image);

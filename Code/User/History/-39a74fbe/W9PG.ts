...
export class CardService {
  ...
  async exportCard(id: number): Promise<any> {
    ...
    this.writeToPage(frontPage, templateInformations.front.informations)
  ...
  private writeToPage(page: PDFPage, template: Record<string, any>, card: Record<string, any>){
    for (const key in template) {
      const value = template[key];

      switch (value.type) {
        case 'string':
            page.drawText(card.data.front[key], {
              x: value.position.x,
              y: value.position.y,
              maxWidth: value.width,
              lineHeight: value.height,
